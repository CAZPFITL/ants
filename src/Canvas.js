...
export default class Canvas extends Screen {
    ...
    static drawBoard() {
        Ants.camera.context.fillStyle = 'transparent'
        Ants.camera.context.fillRect(
            Ants.Helpers.getStepSize(0), //change by random then save it in anthill, then spawn every ant from there
            Ants.Helpers.getStepSize(0),
            Ants.Helpers.getStepSize(1),
            Ants.Helpers.getStepSize(1))
    ...
    static getCanvas() {
        ...
        canvas.width = (window.innerWidth - step) - window.innerWidth.toString().slice(-1)
        canvas.height = (window.innerHeight - step) - window.innerHeight.toString().slice(-1)
